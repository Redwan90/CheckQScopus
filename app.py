...
SCOPUS_API_KEY = 'c9187cfd41d4c6496be780048b47904e' HEADERS_SCOPUS = {'X-ELS-APIKey': SCOPUS_API_KEY}

st.title("OpenAlex Citation Checker with Scopus + SCImago Quartile") input_doi = st.text_input("Enter the DOI of your article:", placeholder="e.g., https://doi.org/10.xxxx/xxxxx")

def clean_doi(doi): return doi.strip().replace("https://doi.org/", "").replace("http://doi.org/", "")

@st.cache_data(show_spinner=False) def get_openalex_citers(doi): openalex_url = f"https://api.openalex.org/works/doi:{doi}" r = requests.get(openalex_url) if r.status_code != 200: return [] work_id = r.json().get("id") if not work_id: return [] cited_url = f"https://api.openalex.org/works?filter=cites:{work_id}&per-page=50" r = requests.get(cited_url) if r.status_code != 200: return [] citing = r.json().get("results", []) return [w.get("doi", "").replace("https://doi.org/", "") for w in citing if w.get("doi")]

@st.cache_data(show_spinner=False) def get_scopus_journal_name(citing_doi): query = f"REF({citing_doi}) OR DOI({citing_doi})" url = f"https://api.elsevier.com/content/search/scopus?query={urllib.parse.quote(query)}&field=source-title" r = requests.get(url, headers=HEADERS_SCOPUS) if r.status_code == 200: entries = r.json().get("search-results", {}).get("entry", []) if entries: return entries[0].get("source-title", "Unknown") return None
...
if input_doi: base_doi = clean_doi(input_doi) with st.spinner("Getting citing DOIs from OpenAlex..."): citing_dois = get_openalex_citers(base_doi) if citing_dois: quartile_summary = {"Q1": 0, "Q2": 0, "Q3": 0, "Q4": 0, "Unknown": 0} citing_records = []
...
else: st.info("Paste a DOI to fetch citing papers and their Scopus/Quartile status.")
