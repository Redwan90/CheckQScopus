#app.py — DOI Citation Checker with Scopus + SCImago Quartile via Semantic Scholar
...
st.title("Semantic Scholar Citation Checker: Scopus Journal + Quartile") input_doi = st.text_input("Enter the DOI of your article:", placeholder="e.g., https://doi.org/10.xxxx/xxxxx")

def clean_doi(doi): return doi.strip().replace("https://doi.org/", "").replace("http://doi.org/", "")

@st.cache_data(show_spinner=False) def get_semantic_citers(doi): url = f"https://api.semanticscholar.org/graph/v1/paper/DOI:{doi}?fields=citations.externalIds" r = requests.get(url) if r.status_code == 200: citations = r.json().get("citations", []) return [c.get("externalIds", {}).get("DOI") for c in citations if c.get("externalIds", {}).get("DOI")] return []
...
if input_doi: base_doi = clean_doi(input_doi) with st.spinner("Fetching citing DOIs from Semantic Scholar..."): citing_dois = get_semantic_citers(base_doi) quartile_summary = {"Q1": 0, "Q2": 0, "Q3": 0, "Q4": 0, "Unknown": 0} citing_records = []
...
for citing_doi in citing_dois:
    ...
    st.subheader("Citing Articles with Quartile Info")
    st.dataframe(df)
    ...
    st.download_button("Download Full Citation Data", df.to_csv(index=False), file_name="citing_articles.csv")
    st.download_button("Download Quartile Summary", q_summary.to_csv(index=False), file_name="quartile_summary.csv")

else: st.info("Enter a DOI to check who cited it and how they're ranked.")
